from helpers import generate_rc_encoded_hash_from_image, \
                    select_random_blocks, create_coeff_pairs_jpg, \
                    embed_with_pairs, \
                    extract_segment
import numpy as np
import cv2
...
def main(image_upload_path, image_download_path, ecc_symbols=20):
    ...
    for row in range(0, h, 8):
        for col in range(0, w, 8):
            ...
            block = y_channel[row:row+8, col:col+8].astype(float) - 128 # Zero shift
            ...
            y_channel[row:row+8, col:col+8] = np.clip(block, 0, 255).astype(np.uint8)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--upload_path", type=str, required=True, help="Path to upload image to process")
    parser.add_argument("--output_path", type=str, default="watermarked_image.jpg", help="Path to output image")
